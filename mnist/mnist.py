...
# Avaliação do Modelo
# Torchmetrics
acc = Accuracy(task="multiclass", num_classes=10).to(device) # Percentual de Acerto
precision = Precision(task="multiclass", num_classes=10).to(device) # Precisão - Quantas vezes ele estava correto
recall = Recall(task="multiclass", num_classes=10).to(device) # Dentre todos os "7" reais, quantos o modelo acertou?
...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        # Transferir predições e labels para CPU se necessário para as métricas
        preds_cpu = preds.cpu()
        labels_cpu = labels.cpu()

        acc.update(preds_cpu, labels_cpu)
        precision.update(preds_cpu, labels_cpu)
        recall.update(preds_cpu, labels_cpu)
